...
def train(batch_size = 14, epochs = 100):
    ...
    cat_list                        = ['bike', 'bus', 'car', 'motor', 'person', 'rider', 'traffic light', 'traffic sign', 'train', 'truck']
    
    # Tensorboard SummaryWriters for training logs
    writer_text                     = SummaryWriter('Tensorboard/cornernet_mobilenetv3_training_text/')
    writer_avg_train_loss           = SummaryWriter('Tensorboard/cornernet_mobilenetv3_training_avg_train_loss_per_epoch/')
    writer_avg_valid_loss           = SummaryWriter('Tensorboard/cornernet_mobilenetv3_training_avg_valid_loss_per_epoch/')
    writer_map_50                   = SummaryWriter('Tensorboard/cornernet_mobilenetv3_training_mean_ap_50/')
    writer_map_75                   = SummaryWriter('Tensorboard/cornernet_mobilenetv3_training_mean_ap_75/')
    writer_cat_ap_50                = SummaryWriter('Tensorboard/cornernet_mobilenetv3_training_categories_ap_50/')
    writer_cat_ap_75                = SummaryWriter('Tensorboard/cornernet_mobilenetv3_training_categories_ap_75/')
    writer_image                    = SummaryWriter('Tensorboard/cornernet_mobilenetv3_training_image/')
    writer_hparams                  = SummaryWriter('Tensorboard/cornernet_mobilenetv3_training_hparams/')

    checkpoint_path = 'NONE'
    '''
    # Loading the best checkpoint
    checkpoint_path                 = '../CornerNet/ModelParams/Hourglass/cornernet_hourglass_pretrained-epoch{}.pth'.format(32)
    ...
    model                           = cornernet(nstack = nstack, out_dim = out_dim).to(device = 'cuda:0')
    ...
    writer_text.add_text(tag = 'MobileNetV3/StartingLogs',                                                                                                \
    ...
    for current_epoch in range(starting_epoch, epochs):
        writer_text.add_text(tag = 'MobileNetV3/RunningLogs', text_string = 'EPOCH: {}/{}'.format((current_epoch + 1), epochs), global_step = (current_epoch + 1), walltime = None)
               
        epoch_since                 = time.time()

        writer_epoch                = SummaryWriter('Tensorboard/cornernet_mobilenetv3_training_avg_loss_per_iteration_epoch_{}/'.format(current_epoch + 1))
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'train':
                ...
                for train_data in train_loader:
                    ...
                    writer_epoch.add_scalar(tag = 'MobileNetV3/TrainingIterationAverageLoss'.format(current_epoch + 1), scalar_value = current_average_train_loss, global_step = current_train_iter)
                ...
                writer_avg_train_loss.add_scalar(tag = 'MobileNetV3/Overfitting', scalar_value = last_epoch_average_train_loss, global_step = (current_epoch + 1))
            ...
            # Validation loop
                ...
                with torch.no_grad():
                    for val_data in val_loader:
                        ...
                        writer_epoch.add_scalar(tag = 'MobileNetV3/ValidationIterationAverageLoss'.format((current_epoch + 1)), scalar_value = current_average_val_loss, global_step = current_val_iter)
                    
                    last_epoch_average_val_loss = current_average_val_loss

                    writer_avg_valid_loss.add_scalar(tag = 'MobileNetV3/Overfitting', scalar_value = last_epoch_average_val_loss, global_step = (current_epoch + 1))
        ...
        if ((current_epoch % mean_ap_epoch_interval) == 0):
            ...
            with torch.no_grad():
                ...
                writer_map_50.add_scalar(tag = 'MobileNetV3/MEAN_AP', scalar_value = current_mAP_50, global_step = (current_epoch + 1))
                writer_map_75.add_scalar(tag = 'MobileNetV3/MEAN_AP', scalar_value = current_mAP_75, global_step = (current_epoch + 1))
                writer_cat_ap_50.add_scalars(main_tag = 'MobileNetV3/CATEGORY_AP_50/', 
                ...
                writer_cat_ap_75.add_scalars(main_tag = 'MobileNetV3/CATEGORY_AP_75/', 
                ...
                writer_image.add_images(tag = 'MobileNetV3/PREDICTIONS/EPOCH_{}'.format(current_epoch + 1), img_tensor = grid_image, global_step = 0, dataformats='CHW') 
        ...
        if((last_epoch_average_val_loss <= best_epoch_average_val_loss) or (last_mAP_50 > best_mAP_50) or (last_mAP_75 > best_mAP_75)):
            ...
            PATH = 'ModelParams/MobileNetV3/cornernet_mobilenetv3_pretrained-epoch{}.pth'.format(current_epoch + 1)                        
            torch.save({
                    'epoch': current_epoch,
            ...
            writer_text.add_text(tag = 'MobileNetV3/SavingLogs',                                                                 \
        ...
        writer_hparams.add_hparams(hparam_dict = {'EPOCH': str(current_epoch + 1),
                                                  ...
                                                  'SAVED': str(is_saved)},
                                    metric_dict = {'MobileNetV3/VV_LEARNING_RATE': base_lr_rate})
        
        epoch_time_elapsed = time.time() - epoch_since

        # Epoch ending logs
        writer_text.add_text(tag = 'MobileNetV3/RunningLogs',                                                                    \
