...
class left_pool(torch.autograd.Function):
    ...
    def forward(self, input_):
        ...
        output = torch.zeros_like(input_, requires_grad=True)
        batch = input_.size(0)
        width = input_.size(3)
...
class CornerPool_module(nn.Module):
    ...
    def forward(self, x):
        ...
        top_pool = self.top_pool(conv3_bn_relu_in_T)

        conv3_bn_relu_in_L = self.conv3_bn_relu_in_L(x)
        left_pool = self.left_pool(conv3_bn_relu_in_L)
        ...
        bottom_pool = self.bottom_pool(conv3_bn_relu_in_B)

        conv3_bn_relu_in_R = self.conv3_bn_relu_in_R(x)
        right_pool = self.right_pool(conv3_bn_relu_in_R)
